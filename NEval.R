...
#   ## Optimization h ##
#   Gamm <- 0.25*K_Q %*% ginv( K_Q/n + lm_Qh*ident_n )
#   alpha1h <- ginv(K_H %*% D1 %*% Gamm %*% D1 %*% K_H+n^2*lm_Hh*K_H) %*%
#     K_H %*% D1 %*% Gamm %*% (data$Y*data$A)
#   alpha0h <- ginv(K_H %*% D0 %*% Gamm %*% D0 %*% K_H+n^2*lm_Hh*K_H) %*%
#     K_H %*% D0 %*% Gamm %*% (data$Y*(1-data$A))
#   ## Optimization q ##
#   Gamm <- 0.25*K_H %*% ginv( K_H/n + lm_Hq*ident_n )
#   alpha1q <- ginv(K_Q %*% D1 %*% Gamm %*% D1 %*% K_Q+n^2*lm_Qq*K_Q) %*%
#     K_Q %*% D1 %*% Gamm %*% matrix(1,n,1)
#   alpha0q <- ginv(K_Q %*% D0 %*% Gamm %*% D0 %*% K_Q+n^2*lm_Qq*K_Q) %*%
...
#     T <- c(w1,x1)
#     S <- rbfkernel(h_arg, sigma = kernel_sigma, matrix(T,1,length(T)))
#     return(t(S) %*% alpha1h)
#   }
...
#     T <- c(z1,x1)
#     S <- rbfkernel(q_arg, sigma = kernel_sigma,matrix(T,1,length(T)))
#     return(t(S) %*% alpha1q)
#   }
